    ...
    <div className="space-y-8 pb-14">
      ...
      <div className="bg-white p-6 rounded-xl border border-[#E8E9EC] shadow-card space-y-5">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-[#F1F5F9] pb-4">
          <div>
            <div className="text-[10px] font-sans font-bold text-[#0F5132] uppercase tracking-wider">
          ...
          <div className="text-xs font-data bg-[#F8F9FB] px-3 py-1.5 rounded-lg border border-[#E8E9EC] text-[#111318]">
        ...
        {!isSizeRegressionValid ? (
          <div className="p-4 bg-[#FFFBEB] border border-[#FDE68A] rounded-xl text-xs text-[#111318] flex items-center gap-3 font-sans">
            <Info className="w-5 h-5 text-[#B45309] shrink-0" />
            <div>
              <div className="font-bold text-[#B45309]">Regression Model Unavailable</div>
          ...
          <>
            <div className="grid grid-cols-2 sm:grid-cols-6 gap-3 text-xs font-sans">
              <div className="p-3 bg-[#F8F9FB] border border-[#E8E9EC] rounded-xl">
                <span className="text-[#6B7280] text-[10px] uppercase tracking-wider font-bold">R-Squared (R²)</span>
                <div className="text-lg font-data tabular-nums font-bold text-[#111318] mt-0.5">{sizeRegression.rSquared}</div>
                <span className="text-[#9CA3AF] text-[10px] font-data">
                  Adj: {sizeRegression.adjustedRSquared?.toFixed(3) ?? sizeRegression.rSquared}
                </span>
              </div>

              <div className="p-3 bg-[#F8F9FB] border border-[#E8E9EC] rounded-xl">
                <span className="text-[#6B7280] text-[10px] uppercase tracking-wider font-bold">Elasticity (β₁)</span>
                <div className="text-lg font-data tabular-nums font-bold text-[#0F5132] mt-0.5">{beta1.toFixed(3)}</div>
                <span className="text-[#9CA3AF] text-[10px] font-data">
              ...
              <div className="p-3 bg-[#F8F9FB] border border-[#E8E9EC] rounded-xl">
                <span className="text-[#6B7280] text-[10px] uppercase tracking-wider font-bold">Constant (β₀)</span>
                <div className="text-lg font-data tabular-nums font-bold text-[#111318] mt-0.5">{beta0.toFixed(3)}</div>
                <span className="text-[#9CA3AF] text-[10px]">Baseline intercept</span>
              </div>

              <div className="p-3 bg-[#F8F9FB] border border-[#E8E9EC] rounded-xl">
                <span className="text-[#6B7280] text-[10px] uppercase tracking-wider font-bold">t-Statistic</span>
                <div className="text-lg font-data tabular-nums font-bold text-[#111318] mt-0.5">{slopeCoef?.tStat ?? '0'}</div>
                <span className="text-[#9CA3AF] text-[10px] font-data">df={sizeRegression.dfResidual ?? 'N/A'}</span>
              </div>

              <div className="p-3 bg-[#F8F9FB] border border-[#E8E9EC] rounded-xl">
                <span className="text-[#6B7280] text-[10px] uppercase tracking-wider font-bold">F-Statistic</span>
                <div className="text-lg font-data tabular-nums font-bold text-[#111318] mt-0.5">
              ...
              <div className="p-3 bg-[#F0FDF4] border border-[#DCFCE7] rounded-xl">
                <span className="text-[#0F5132] text-[10px] uppercase tracking-wider font-bold">p-Value (Slope)</span>
                <div className="text-lg font-data tabular-nums font-bold text-[#0F5132] mt-0.5">
                  {(slopeCoef?.pValue ?? 0) < 0.001 ? '< 0.001' : (slopeCoef?.pValue ?? 0).toFixed(3)}
                </div>
                <span className="text-[#0F5132]/80 text-[10px] font-semibold">Significant (p&lt;0.05)</span>
              </div>
            </div>

            {/* Empirical Interpretation Box */}
            <div className="p-4 bg-[#F8F9FB] border border-[#E8E9EC] rounded-xl text-xs text-[#4B5563] space-y-1 font-sans">
              <div className="font-bold text-[#111318] flex items-center gap-1.5 text-xs uppercase tracking-wider">
          ...
          <ResponsiveContainer width="100%" height="100%">
            <ComposedChart margin={{ top: 10, right: 30, left: 10, bottom: 25 }}>
              ...
              <XAxis
                ...
                label={{ value: 'Net Package Weight (Grams)', position: 'insideBottom', offset: -15, fontSize: 11, fill: '#6B7280' }}
              />
              <YAxis
                ...
                tick={{ fontSize: 11, fill: '#6B7280' }}
                axisLine={{ stroke: '#E8E9EC' }}
                label={{ value: 'Price / 100g (₱)', angle: -90, position: 'insideLeft', offset: 0, fontSize: 11, fill: '#6B7280' }}
              />
              <Tooltip
                content={({ active, payload }) => {
                  if (active && payload && payload.length) {
                    ...
                    return (
                      <div className="bg-white text-[#111318] p-3 rounded-xl shadow-card border border-[#E8E9EC] text-xs space-y-1 font-sans">
                        <div className="font-bold text-[#111318]">{d.brand}</div>
                        <div className="text-[#6B7280]">{d.name}</div>
                        <div>
                          Size: <strong className="font-data">{d.size}g</strong>
                        </div>
                        <div>
                          Price/100g: <strong className="font-data tabular-nums text-[#0F5132]">₱{Number(d.pricePer100g).toFixed(2)}</strong>
                        </div>
                      </div>
                    );
                  }
              ...
              />
              <Scatter data={scatterPoints} fill="#0F5132" opacity={0.65} />
              <Line
                data={linePoints}
                dataKey="trend"
                stroke="#B45309"
      ...
      <div className="bg-white p-6 rounded-xl border border-[#E8E9EC] shadow-card space-y-5">
        ...
        {hedonicRegression.status && hedonicRegression.status !== 'valid' ? (
          <div className="p-4 bg-[#FFFBEB] border border-[#FDE68A] rounded-xl text-xs text-[#111318] flex items-center gap-3 font-sans">
            <Info className="w-5 h-5 text-[#B45309] shrink-0" />
            <div>
              <div className="font-bold text-[#B45309]">Hedonic Regression Model Unavailable</div>
              <div className="text-[#6B7280] mt-0.5">
          ...
          <>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-xs font-sans">
              <div className="p-3 bg-[#F8F9FB] border border-[#E8E9EC] rounded-xl">
                <span className="text-[#6B7280] text-[10px] uppercase tracking-wider font-bold">Model R² / Adj R²</span>
                <div className="text-base font-data tabular-nums font-bold text-[#111318] mt-0.5">
                  {hedonicRegression.rSquared} / {hedonicRegression.adjustedRSquared?.toFixed(3) ?? hedonicRegression.rSquared}
                </div>
                <span className="text-[#9CA3AF] text-[10px]">Variance explained</span>
              </div>
              ...
              <div className="p-3 bg-[#F8F9FB] border border-[#E8E9EC] rounded-xl">
                <span className="text-[#6B7280] text-[10px] uppercase tracking-wider font-bold">Degrees of Freedom</span>
                <div className="text-base font-data tabular-nums font-bold text-[#0F5132] mt-0.5">
                  df = {hedonicRegression.dfResidual ?? 'N/A'}
                </div>
                <span className="text-[#9CA3AF] text-[10px]">Residual error df</span>
              </div>

              <div className="p-3 bg-[#F8F9FB] border border-[#E8E9EC] rounded-xl">
                <span className="text-[#6B7280] text-[10px] uppercase tracking-wider font-bold">Estimator</span>
                <div className="text-base font-sans font-bold text-[#111318] mt-0.5">
                  Log-Linear OLS
                </div>
                <span className="text-[#9CA3AF] text-[10px]">Gauss-Jordan Partial Pivot</span>
              </div>
            </div>
                  ...
                  {hedonicRegression.coefficients.map((c, i) => {
                    ...
                    return (
                      <tr key={i} className="hover:bg-[#F8F9FB]">
                        <td className="py-2.5 px-4 font-bold text-[#111318]">{c.label}</td>
                        <td className="py-2.5 px-3 text-right font-data tabular-nums text-[#6B7280]">
                        ...
                        <td className="py-2.5 px-3 text-right font-data tabular-nums text-[11px] text-[#6B7280]">
                          {c.pValue !== undefined
                            ? c.pValue < 0.001
                              ? '< 0.001'
                              : c.pValue.toFixed(3)
                        ...
                        <td className="py-2.5 px-4 text-[#6B7280] text-[11px]">{c.interpretation}</td>
                      </tr>
                    );
                  })}
                </tbody>
      ...
      <div className="bg-white rounded-xl text-[#111318] p-6 sm:p-8 shadow-card border border-[#E8E9EC] space-y-6">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-[#F1F5F9] pb-4">
          <div>
            <div className="inline-flex items-center gap-1.5 text-xs text-[#0F5132] font-sans font-bold uppercase tracking-wider mb-1">
              <Sparkles className="w-3.5 h-3.5 text-[#0F5132]" />
              <span>Applied Household Economics Simulator</span>
            </div>
            <h3 className="text-xl font-bold text-[#111318] tracking-tight font-sans">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 font-sans">
          <div className="space-y-2">
            <div className="flex justify-between text-xs">
              <span className="text-[#6B7280] font-medium">Household Members:</span>
              <strong className="text-[#0F5132] font-data tabular-nums font-bold">{householdSize} persons</strong>
            </div>
            <input
          ...
          <div className="space-y-2">
            <div className="flex justify-between text-xs">
              <span className="text-[#6B7280] font-medium">Brushings per Day:</span>
              <strong className="text-[#0F5132] font-data tabular-nums font-bold">{brushingsPerDay} times / day</strong>
            </div>
            <input
          ...
          <div className="space-y-2">
            <div className="flex justify-between text-xs">
              <span className="text-[#6B7280] font-medium">Portion per Brush:</span>
              <strong className="text-[#0F5132] font-data tabular-nums font-bold">{toothpasteGramPerBrush} grams</strong>
            </div>
            <input
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 pt-2 font-sans">
          ...
          <div className="p-4 bg-[#F8F9FB] border border-[#E8E9EC] rounded-xl space-y-1">
            <span className="text-[10px] text-[#6B7280] font-sans font-bold uppercase tracking-wider">
              Strategy A: Small Tubes (&le;100g)
            </span>
            <div className="text-xl font-data tabular-nums font-bold text-[#111318]">₱{annualCostSmall.toFixed(2)} / yr</div>
            <p className="text-[10px] text-[#6B7280] font-data">
          ...
          <div className="p-4 bg-[#F8F9FB] border border-[#E8E9EC] rounded-xl space-y-1">
            ...
            <div className="text-xl font-data tabular-nums font-bold text-[#0F5132]">₱{annualCostLarge.toFixed(2)} / yr</div>
            <p className="text-[10px] text-[#0F5132] font-semibold font-data">
              Save ₱{annualSavingsLarge.toFixed(2)} / year
            </p>
          </div>

          {/* Option C: Twin / Multi-Packs */}
          <div className="p-4 bg-[#F0FDF4] border border-[#DCFCE7] rounded-xl space-y-1">
            <span className="text-[10px] text-[#0F5132] font-sans font-bold uppercase tracking-wider">
              Strategy C: Twin / Value Packs
            </span>
            <div className="text-2xl font-data tabular-nums font-bold text-[#0F5132]">
              ₱{annualCostTwin.toFixed(2)} / yr
            </div>
            <p className="text-xs text-[#0F5132] font-bold font-data">
              Save ₱{annualSavingsTwin.toFixed(2)} / year ({((annualSavingsTwin / annualCostSmall) * 100).toFixed(0)}% lower)
            </p>
          </div>
        </div>

        <div className="text-[10px] text-[#6B7280] pt-2 border-t border-[#F1F5F9] flex justify-between font-sans">
          <span>
            Household uses ~<strong className="text-[#111318] font-data">{(annualConsumptionGrams / 1000).toFixed(2)} kg</strong> toothpaste per year
          </span>
